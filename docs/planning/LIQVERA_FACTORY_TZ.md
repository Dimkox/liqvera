...
**Версия:** 1.1 (бренд Liqvera и статус публикации) · **Дата:** 24 сентября 2026 года  
...
**F0 выполнен:** технический снимок опубликован с новой Git-историей, локальным сканированием, очисткой личных данных и проверкой анонимного клонирования. Исходник остался приватным, его `main` не изменён. Проект получил утверждённый бренд Liqvera; полное приложение, `make verify` и on-chain оплата ещё не проверены. Результаты переноса описаны в [PROVENANCE](../../PROVENANCE.md), актуальный следующий этап — F1.
...
**Статус:** выполнен для Liqvera. Критерии ниже сохранены для проверки и воспроизведения переноса.
...
8. Создать `Dimkox/liqvera`; до первого push отключить выполнение GitHub Actions на destination, если workflow присутствуют в snapshot. Не переносить secrets/environments. Если имя занято, проверить владельца и provenance; не перезаписывать чужой или неожиданный репозиторий.
