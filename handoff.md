...
Выполнить F1 из [ТЗ](docs/planning/LIQVERA_FACTORY_TZ.md): проверить baseline-тесты, метаданные инструмента, Mezo Testnet и платёжную конфигурацию. После этого реализовать F2–F7: контракты → проверяемый отчёт → API/хранение → тестовая MUSD-оплата → интерфейс → приёмка.
