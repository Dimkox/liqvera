...
Следующее действие после письменного review дизайна — подготовить детальный
план F1: исправить воспроизводимость публичного baseline, закрепить toolchain и
SDK compatibility, принять узкий ADR и сохранить полный результат проверки.
После F1 реализовать F2–F7: контракты → проверяемый отчёт → API/хранение →
тестовая MUSD-оплата → интерфейс → приёмка.
