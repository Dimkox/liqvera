...
Проверены происхождение файлов, новая история, локальные сканы и отсутствие секретов в публикуемом снимке с учётом рассмотренных ложных срабатываний. Публикация проверяется по публичной metadata, анонимному clone и совпадению дерева. Это проверка переноса; полный `make verify`, Docker и on-chain оплата не выполнялись.
